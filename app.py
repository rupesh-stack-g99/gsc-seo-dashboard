...
COMPARISON_MODE = st.sidebar.selectbox(
    ...
    options=["3-Month Comparison", "Normal 28 Days"],
    index=0,
    help="Select '3-Month Comparison' to parse performance differentials or 'Normal 28 Days' to isolate static ranking opportunities."
)
...
with st.expander("📖 View Forensic Capability & Core Functionality", expanded=False):
    ...
    * **What it finds:**
        * **Systemic Core Update Impact (3-Month Mode only):** Calculates the exact balance of search term decay vs. growth to flag potential sitewide algorithmic updates.
        * **Hidden Keyword Decay (3-Month Mode only):** Pinpoints queries losing substantial click-through metrics while keeping stable impression scores.
        ...
        * **Hidden Growth (Flat Traffic - 3-Month Mode only):** Pinpoints pages generating high visibility gains (impressions) without generating corresponding click increases.
...
if COMPARISON_MODE == "3-Month Comparison":
    ...
    <div class="upload-requirements-box">
        <h3 style="margin-top:0; color: #4f46e5 !important;">⚠️ GSC Export Requirement Checklist: 3-Month Comparison</h3>
    ...
    st.markdown("""
    <div class="upload-requirements-box" style="border-color: #10b981; background-color: rgba(16, 185, 129, 0.05);">
        <h3 style="margin-top:0; color: #10b981 !important;">📅 GSC Export Requirement Checklist: Normal 28 Days</h3>
        <p style="margin-bottom:8px; font-size:0.95rem;">To analyze normal 28-day snapshots, download your standard performance metrics:</p>
        <ul style="margin-top:0; margin-bottom:0; font-size:0.95rem; line-height: 1.6;">
            <li>Go to Google Search Console performance menu.</li>
            <li>Set your Date filter range to: <b>Last 28 days</b> (no comparative check required).</li>
            <li>Click the <b>Export</b> button in the top right corner and choose <b>Download ZIP</b>.</li>
            <li>Upload that unaltered ZIP archive below.</li>
        </ul>
    </div>
...
if uploaded_file is not None:
    ...
    if gsc and 'Queries' in gsc and 'Pages' in gsc:
        ...
        # Calculate Delta Health Metrics (Only validated if comparative columns exist)
        has_comparison_metrics = 'Clicks_Delta' in df_q.columns and (df_q['Clicks_Delta'].abs().sum() > 0)
        
        # Configurable tab arrays depending on data availability
        tab_names = [
        ...
        with tabs[1]:
            ...
            if COMPARISON_MODE == "Normal 28 Days" or not has_comparison_metrics:
                st.info("ℹ️ **This tab is optimized for '3-Month Comparison' datasets.** Switching GSC files to a comparative layout enables tracking of specific keyword losses over time.")
            else:
                st.markdown("""
